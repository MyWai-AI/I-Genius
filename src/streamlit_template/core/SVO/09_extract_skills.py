...
from _paper_config import HANDS_PATH, PREPOST_DELTA_P, RELEASE_MIN_MOVE_FRAMES, SEG_PATH, TRAJ_PATH, ensure_output_dirs
...
def main() -> None:
    ...
    reach = hand[:grasp_idx]
    move = hand[grasp_idx + 1 : release_idx]
    ...
    np.save(TRAJ_PATH / 'reach_traj.npy', reach)
    np.save(TRAJ_PATH / 'move_traj.npy', move)
    np.save(TRAJ_PATH / 'grasp_pos.npy', grasp_pos)
    np.save(TRAJ_PATH / 'release_pos.npy', release_pos)
    np.save(TRAJ_PATH / 'pre_grasp_pos.npy', pre_grasp)
    np.save(TRAJ_PATH / 'post_grasp_pos.npy', post_grasp)
    np.save(TRAJ_PATH / 'pre_release_pos.npy', pre_release)
    np.save(TRAJ_PATH / 'post_release_pos.npy', post_release)
