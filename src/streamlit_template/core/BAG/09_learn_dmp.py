...
def plot_forcing(path: Path, forcing_target: np.ndarray, forcing_rollout: np.ndarray, title: str) -> None:
    ...
    axes = ['X', 'Y', 'Z']
    for d in range(3):
        plt.plot(forcing_target[:, d], label=f'{axes[d]} target', alpha=0.5)
        plt.plot(forcing_rollout[:, d], '--', label=f'{axes[d]} learned')
    plt.xlabel('Timestamp [frame]')
...
def train_phase(name: str, traj: np.ndarray) -> dict[str, float]:
    ...
    plot_forcing(
        PLOTS_PATH / f'{name}_forcing_term.png',
        forcing_target=diag['f_target'],
        forcing_rollout=rollout['f'],
        title=f'{name.capitalize()} DMP Forcing Term',
    )
    plot_reproduction(
        PLOTS_PATH / f'{name}_dmp_reproduction.png',
        demo=traj,
        reproduction=y_rep,
        title=f'{name.capitalize()} DMP Demonstration vs Reproduction',
    )
...
def main() -> None:
    ...
    reach = np.load(SEG_PATH / 'reach_traj.npy')
    move = np.load(SEG_PATH / 'move_traj.npy')
