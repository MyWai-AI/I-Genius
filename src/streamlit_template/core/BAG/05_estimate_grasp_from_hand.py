...
def main() -> None:
    ...
    hand_tips = np.load(HANDS_PATH / 'hand_tips_3d_smooth.npy')  # [T,5,3]
    object_traj = np.load(OBJECTS_PATH / 'object_3d_smooth.npy')  # [T,3]
    hand_center = np.load(HANDS_PATH / 'hand_3d_smooth.npy')  # [T,3]
    ...
    if not np.any(finite):
        # Fallback only when fingertip reconstruction completely fails.
