...
def adapt_phase(name: str, demo: np.ndarray, new_start: np.ndarray, new_goal: np.ndarray) -> np.ndarray:
    ...
    plot_adaptation(
        PLOTS_PATH / f'{name}_adaptation.png',
        demo=demo,
        repro=repro,
        adapted=adapted,
        new_start=new_start,
        new_goal=new_goal,
        title=f'Learned and Updated DMP for {name.capitalize()}',
    )
    return adapted
...
def main() -> None:
    ...
    reach_demo = np.load(SEG_PATH / 'reach_traj.npy')
    move_demo = np.load(SEG_PATH / 'move_traj.npy')

    pre_grasp = np.load(SEG_PATH / 'pre_grasp_pos.npy')
    post_grasp = np.load(SEG_PATH / 'post_grasp_pos.npy')
    pre_release = np.load(SEG_PATH / 'pre_release_pos.npy')

    # Reach: current EE position -> pre-grasp position
    reach_new_start = reach_demo[0] + np.array([0.03, -0.01, 0.00], dtype=np.float32)
    reach_new_goal = pre_grasp

    # Move: post-grasp -> pre-release
