...
def handle_svo_objects(
    ...
    with st.spinner("Extracting 3D object trajectory..."):
        ...
        # Choose model: respect explicit model path first; otherwise preserve legacy SVO behavior.
        ...
        for fname in rgb_files:
            ...
            if _use_obb:
                # --- OBB model (unchanged — OBB doesn't support model.track()) ---
                shaft_boxes = []
                results = model(bgr, verbose=False, conf=_conf, imgsz=1280, max_det=100)[0]
                    ...
                    if _tracking_mode and _target_center is not None:
                        ...
                        for _si in shaft_boxes:
                            if _target_area is not None and _target_area > 1.0:
                                ...
                                _size_ok.append(_si)
                        _candidates_pool = _size_ok if _size_ok else shaft_boxes
                        best_idx = _candidates_pool[int(np.argmin([
                            float(np.hypot(_shaft_xywhr[_si][0] - tx, _shaft_xywhr[_si][1] - ty))
                            for _si in _candidates_pool
                        ]))]
                    else:
                        best_idx = shaft_boxes[int(np.argmax(confs[shaft_boxes]))]
                    if shaft_boxes:
                        xywhr = results.obb.xywhr[best_idx].cpu().numpy()
                        cx_b, cy_b, w_b, h_b, rot_b = xywhr
                        rect = ((float(cx_b), float(cy_b)), (float(w_b), float(h_b)), float(np.degrees(rot_b)))
                        bp = cv2.boxPoints(rect)
                        bp = np.int0(bp)
                        cv2.drawContours(bgr, [bp], 0, (0, 0, 255), 2)
                        cv2.putText(bgr, f"{float(confs[best_idx]):.2f}", (int(cx_b), int(cy_b)),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                        u_i, v_i = int(round(float(cx_b))), int(round(float(cy_b)))
                        z = median_valid_depth(depth, u=u_i, v=v_i, half=2)
                        if z is not None:
                            trajectory_3d.append(to_camera_xyz(u_i, v_i, z, intr))
                            detected = True
                            total_det += 1
                            _target_center = [float(cx_b), float(cy_b)]
            else:
                # --- Standard detector: same predict-then-filter approach as Local page ---
                # Use model.predict() every frame (no BoT-SORT). In tracking mode, filter
                # candidates by label + size + proximity to last known center, exactly as
                # the Local page does when the user selects an object.
                results = model(bgr, verbose=False, conf=_conf, imgsz=1280,
                ...
                if results.boxes is not None and len(results.boxes) > 0:
                    ...
                    _frame_h, _frame_w = bgr.shape[:2]
                    _frame_area = float(_frame_h * _frame_w)

                    # Build candidates filtered by area bounds
                    _candidates = []
                    ...
                    if _candidates:
                        if _tracking_mode and _target_center is not None:
                            # Filter by label then size. If nothing passes, the object is occluded —
                            # don't pick a wrong object, just leave _selected=None for this frame.
                        ...
                        _color = (0, 0, 255) if _tracking_mode else (0, 200, 0)
                        cv2.rectangle(bgr, (x1, y1), (x2, y2), _color, 2)
                        cv2.putText(bgr, f"obj {_selected['conf']:.2f}", (x1, max(y1 - 6, 10)),
...
def handle_svo_trajectory(base_path: Path, session_id: str = None):
    ...
    with st.spinner("Estimating grasp point..."):
        ...
        # Skip leading zero-padded frames (no detection yet) before averaging.
        ...
        # Distance from each hand frame to the fixed initial object position
        ...
        # Smooth then find the FIRST local minimum (not the global argmin).
        # The global argmin can pick a second pass near the initial position
        # long after the actual grasp.  The first significant dip is the real grasp.
        distances_smooth = _uf1d(distances.astype(np.float64), size=5).astype(np.float32)

        # Adaptive distance threshold: the hand must be within this distance
        # of the initial position to be considered a true approach.
        _dist_thresh = max(0.03, float(np.percentile(distances_smooth[finite], 5)))

        # Scan for the first local minimum that is below the threshold.
        grasp_idx = int(np.argmin(distances_smooth))  # fallback: global argmin
        for i in range(1, t_len - 1):
            if distances_smooth[i] <= _dist_thresh:
                # Check if this is a local minimum (or start of a low plateau)
                if distances_smooth[i] <= distances_smooth[i - 1] and distances_smooth[i] <= distances_smooth[i + 1]:
                    grasp_idx = i
                    break
                # Also accept start of a low-distance plateau
                if i >= 2 and distances_smooth[i] <= distances_smooth[i - 2] * 1.1:
                    grasp_idx = i
                    break

        # If argmin lands at frame 0, initial-position estimate may be off;
        # fall back to distance-to-current-position argmin.
        if grasp_idx == 0:
    ...
    with st.spinner("Running GMM segmentation (release detection)..."):
        ...
        # Identify last (post-placing) cluster by proximity to final position
        # Use last 10% of frames, but only nonzero ones to avoid zero-pad contamination.
        n_final = max(5, int(len(reconstructed) * 0.10))
        ...
        # Compute velocity for low-velocity guard
        vel = np.linalg.norm(np.diff(reconstructed, axis=0), axis=1)
        vel = np.concatenate([[vel[0] if len(vel) else 0.0], vel])
        low_vel_threshold = float(np.percentile(vel, 40))

        # Find release: first stable window in last cluster AND low velocity.
        # The velocity guard prevents a false release when the object bbox
        # jumps/drifts while the hand is still actively moving.
        ...
        # Fallback: relax velocity requirement, cluster membership only
