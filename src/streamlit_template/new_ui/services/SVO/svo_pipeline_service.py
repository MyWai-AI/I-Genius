...
def handle_svo_trajectory(base_path: Path, session_id: str = None):
    ...
    with st.spinner("Estimating grasp point..."):
        ...
        # Skip leading zero-padded/non-finite frames before averaging.
    ...
    with st.spinner("Running GMM segmentation (release detection)..."):
        ...
        # Sanity: cross-reference with tail-window vote
        tail_window = labels[max(0, n_rec - 20):]
        tail_vote_cluster = int(np.bincount(tail_window).argmax())
        if last_cluster != tail_vote_cluster:
            pass  # Use final-position cluster (paper-aligned); tail vote is only informational
