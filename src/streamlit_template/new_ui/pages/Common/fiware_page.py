...
DEFAULT_FIWARE_DISCOVERY_SERVER = ""
...
def _init_fiware_state() -> None:
    ...
    st.session_state.setdefault(
        ...
        "Trajectory published through I-Genius and received by the edge runtime.",
    )
    st.session_state.setdefault(
        "fiware_notes",
...
def render_fiware_page() -> None:
    ...
    with col_title:
        st.markdown(
            "<h2 style='margin: 0; padding-top: 0;'>🏭 FIWARE Integration</h2>",
    ...
    st.caption(
        "For a local Orion-LD broker, start `docker compose -f docker-compose.fiware.yml up -d` "
        "and keep `http://localhost:1026` in the broker field."
    )

    st.markdown(
        """
        This page gives I-Genius a minimal ARISE-aligned API surface:

        - `I-Genius UI` prepares and triggers the trajectory execution.
    ...
    with right_col:
        ...
        st.caption(
            "This entity can be stored in Orion-LD to track the IT-level lifecycle "
            "of a trajectory execution."
        )
