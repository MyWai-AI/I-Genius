"""
BAG Pipeline Page - Dedicated page for running the BAG processing pipeline.
Same stepper UI as Generic pipeline, using BAG-specific service handlers.
"""
...
from src.streamlit_template.new_ui.services.BAG.bag_pipeline_service import (
    handle_bag_hands,
    handle_bag_objects,
    handle_bag_trajectory,
    handle_bag_dmp,
    handle_bag_robot,
)
...
def _get_bag_data_paths():
    """Get BAG pipeline data paths."""
    base = Path("data") / "BAG" # Root
...
def render_bag_pipeline_page():
    """Render the BAG pipeline processing page."""
    ...
    clicked_step = stepper.display(statuses=statuses, key_prefix="bag_pipeline_", clickable=not is_running)
            ...
            try:
                ...
                if next_step == 0:
                    ...
                    if _local_src_hash == sess and isinstance(_shared_bbox, (list, tuple)) and len(_shared_bbox) == 4:
                        _bag_bbox = list(_shared_bbox)
                    elif isinstance(_legacy_bbox, (list, tuple)) and len(_legacy_bbox) == 4:
                        _bag_bbox = list(_legacy_bbox)
                    else:
                        _bag_bbox = None
                    _bag_label = st.session_state.get("selected_tracking_label") if _local_src_hash == sess else None
                    _bag_size_ratio = float(st.session_state.get("selected_object_bbox_size_ratio", 4.0))
                    handle_bag_objects(base, session_id=sess, tracking_bbox_xyxy=_bag_bbox, tracking_label=_bag_label, bbox_size_ratio=_bag_size_ratio)
                elif next_step == 2:
                    handle_bag_trajectory(base, session_id=sess)
                elif next_step == 3:
                    handle_bag_dmp(base, session_id=sess)
                elif next_step == 4:
                    u_path, r_config = _resolve_active_robot()
                    handle_bag_robot(base, session_id=sess, urdf_path=u_path, robot_config=r_config)
        ...
        if os.path.exists(urdf_path_local):
            ...
            if traj_points:
                with st.popover("Action", use_container_width=False):
                    ...
                    if st.button("Generate New Trajectory", key="bag_save_result", type="primary"):
            ...
            if video_path and os.path.exists(video_path):
                ...
                sync_viewer(video_path=video_src, viewer_data=viewer_data, key="sync_v_bag_robot", autoplay=False)
            else:
                sync_viewer(viewer_data=viewer_data, video_path=None, key="sync_v_bag_robot_novid")
        else:
    ...
    if rtype == "dmp3d":
        ...
        if traj_data:
            ...
            sync_viewer(video_path=video_src, viewer_data=viewer_data, key="sync_v_bag_dmp")
            return
    ...
    with right:
        ...
        if rtype in ["hands", "objects"]:
            ...
            ready = _frame_viewer.sync_timeline_touch_state(img_paths, key_prefix=f"bag_pipeline_{rtype}") if hasattr(_frame_viewer, "sync_timeline_touch_state") else True
            if not ready:
                st.info("Adjust the timeline range to show frames.")
                if hasattr(_frame_viewer, "render_timeline_range_control"):
                    _frame_viewer.render_timeline_range_control(img_paths, key_prefix=f"bag_pipeline_{rtype}")
                return

            display_paths = _frame_viewer.timeline_trim_paths(img_paths, key_prefix=f"bag_pipeline_{rtype}")
            if not display_paths:
                return

            encoded = encode_images(tuple(display_paths))
            grid_key = f"bag_viewer_grid_{selected_step}_{len(display_paths)}"
