...
def render_bag_pipeline_page():
    ...
    if rtype == "robot3d":
        ...
        if os.path.exists(urdf_path_local):
            ...
            if video_path and os.path.exists(video_path):
                ...
                sync_viewer(video_path=video_src, viewer_data=viewer_data, key="sync_v_bag_robot")
            else:
            ...
            if traj_points:
                ...
                with st.popover("Action", use_container_width=False):
                    ...
                    if transport == TRANSPORT_CYCLONEDDS:
                        dds_topic = st.text_input("Topic", value="/joint_trajectory", key="bag_dds_topic")
                    else:
                        ...
                        status_wait_sec = st.number_input(
                            ...
                            max_value=60.0,
                            step=1.0,
                            key="bag_vulcanexus_status_wait",
                            help="Set to 0 to skip waiting for the edge executor status.",
                        )
                    if st.button("🤖 Push to Robot", key="bag_push_robot"):
                        if transport == TRANSPORT_CYCLONEDDS:
    ...
    with right:
        ...
        if rtype in ["hands", "objects"]:
            ...
            grid_key = f"bag_viewer_grid_{selected_step}_{len(img_paths)}"
