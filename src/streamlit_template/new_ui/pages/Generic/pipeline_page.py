...
def _resolve_active_robot():
    ...
    # Check custom upload — prefer custom_robot_dir (set by the external source page), fallback to active_custom_robot_dir
    _custom = (
    ...
    if is_running:
        ...
        if statuses[next_step] == "running":
            try:
                ...
                if next_step == 0:
                    # Check if frames extracted. If not (e.g. from external), extract them now.
    ...
    if rtype == "robot3d":
        ...
        if os.path.exists(urdf_path_local):
            ...
            if video_path:
                    ...
                    sync_viewer(video_path=video_src, viewer_data=viewer_data, key=f"sync_v_robot")
            else:
